    ...
    if (optimize && probeLevel > 0) {
      ...
      if (!optRes || optRes.error) {
        ...
        if ((!Array.isArray(optPdf) || !optPdf.length ||
          ...
          Logger.log('OPTIMIZE: Fallback to baseline points applied (revert case)');
        }
        ...
        Logger.log('OPTIMIZE: Attached reshapedPoints to envelope: PDF length=' + optimizeBlock.reshapedPoints.pdfPoints.length);
...
function projectcareAPI(tasks) {
  try {
    ...
    for (var ti = 0; ti < tasks.length; ti++) {
      if (tasks[ti] && tasks[ti].cpmOptions && typeof tasks[ti].cpmOptions === 'object') {
        cpmOptions = tasks[ti].cpmOptions;
        break;
      }
    }
    ...
    // CP Engine: run if any task declares predecessors
    var cpEngineResult = null;
    var hasDependencies = tasks.some(function(t) {
    ...
    if (hasDependencies) {
      ...
      try {
        ...
        cpEngineResult = {
          status:  'error',
          message: (cpe && cpe.message) ? cpe.message : 'CP engine invocation failed'
        };
      }
    }
    ...
    return { results: [], cpEngine: null, error: message, details, feedbackMessages: [message] };
  }
}
